...
public class GreetingCommand
{
	...
	{
		source.sendFeedback(new TranslationTextComponent("commands.opst.creator.greet", source.getName()), true);
		return 1;
	}

	private static int printGreetMsg(CommandSource source)
	{
		source.sendFeedback(new TranslationTextComponent("commands.opst.creator.greet.Dev", source.getEntity().getName().getString()), true);
		return 1;
	}
}
